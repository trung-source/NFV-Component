...
def setvni(ip_src='',ip_dst='',port_dst='',route='',vni=''):
    check = 10
    host_ip = ip_src
    dst_ip = ip_dst
    ...
    if str(host_ip) == '':
        print("Dia chi Host cua Pod khong ton tai!")
...
def upload_file():
    if request.method == 'POST':
        ...
        ipv4_src_host = data["ip_host_src"]
        ipv4_dst_host = data["ip_host_dst"]
            ...
            for item in db_route:
                if ip_src == item[0] and ip_dst == item[1]:
                    if port_dst == item[2]:
                        ...
                        vni = item[4]
                        port_out = setvni(ipv4_src_host,ipv4_dst_host,port_dst,route,vni)
                        setvni(ipv4_dst_host,ipv4_src_host,port_src,route_reverse,vni)
                        mydb.commit()
                        mydb.close()
...
def update_route():
    ...
    db_route = mycursor.fetchall()
    for item in db_route:
        if ip_src == item[0] and ip_dst == item[1] and port_dst == item[2] and data['route'] == item[3] and data['vni'] == item[4]:
            check = True
            mydb.commit()
            mydb.close()
            return "Ton tai trong Database"
    ...
    if check == False:
        if (update==True):
            ...
            add_route= "UPDATE Route SET vni = '%s' WHERE ip_src = '%s' AND ip_dst = '%s' AND port_dst = '%s'" % (data['vni'], item[0],ip_dst, port_dst)
            mycursor.execute(add_route)
            # flow(ip_src,ip_dst,port_dst,route)
            route_reverse = route[::-1]
            ...
            ipv4_dst_host = ''
            getUdateData()
            mycursor.execute('select host_ip from pod.Pods where ip_pod = "'+ip_src+'"')
            db_route = mycursor.fetchall()
            for item in db_route:
                ipv4_src_host = item[0]

            mycursor.execute('select host_ip from pod.Pods where ip_pod = "'+ip_dst+'"')
            db_route = mycursor.fetchall()
            for item in db_route:
                ipv4_dst_host = item[0]

            setvni(ipv4_src_host,ipv4_dst_host,port_dst,route,vni)
            # setvni(ipv4_dst_host,ipv4_src_host,port_src,route_reverse,vni)
            ...
            add_route = "INSERT INTO Route (ip_src, ip_dst, port_dst, route, vni) VALUES ('%s','%s','%s','%s','%s')" % (ip_src, ip_dst, port_dst, route, vni) 
            mycursor.execute(add_route)
            # flow(ip_src,ip_dst,port_dst,route)
            route_reverse = route[::-1]
            # flow1(ip_dst,ip_src,port_dst,route_reverse)
            ...
            # print("IP pod nguon: " + ip_src)
            # print("IP pod dich: " + ip_dst)
            # print("IP host nguon: " + ipv4_src_host)
            # print("IP host dich: " + ipv4_dst_host)
            setvni(ipv4_src_host,ipv4_dst_host,port_dst,route,vni)
            # setvni(ipv4_dst_host,ipv4_src_host,port_src,route_reverse,vni)
    ...
    mydb.close()
    return "Hoan Thanh"
@app.route("/readforsubcontroller" , methods = ['POST'])
def read_for_subcontroller():
    if request.method == 'POST':
        mydb = mysql.connector.connect(
        host="127.0.0.1",
        user="core",
        ...
        if ip_dst in db_pods and ip_src in db_pods :
            ...
            for item in db_route:
                if ip_src == item[0] and ip_dst == item[1]:
                    if int(port_dst) == int(item[2]):
                        # print("vaotrongbinhthuong")
                        route = item[3]
                        route_reverse = route[::-1]
                        vni = item[4]
                        # print(vni)

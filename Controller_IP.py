...
class ProjectController(app_manager.RyuApp):
    ...
    def _switch_features_handler(self, ev): # khi khoi tao Ryu
        ...
        match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,ipv4_src='10.2.0.2', ipv4_dst='10.2.0.4',ip_proto=17,udp_dst=4789)
        actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,
                                          ofproto.OFPCML_NO_BUFFER)]
        self.add_flow_notimeout(datapath, 30000, match, actions)
    ...
    def _packet_in_handler(self, ev):
        ...
        try:
            if pkt.protocols[3].vni != 4096:
                self.logger.info("Tiep nhan goi tin VXLAN: %s", pkt.protocols[3].vni)
            else:
                return
        except:
            self.logger.info("")
        ...
        if isinstance(ip_pkt,ipv4.ipv4):
            ...
            if dst_ip in self.arp_table:
                ...
                if check == True:
                    dict_key = ["ip_src", "ip_dst","ip_host_src","ip_host_dst","port_src", "port_dst"]
                    ipv4_dst_host = pkt.protocols[1].dst
                    ipv4_src_host = pkt.protocols[1].src
                    ipv4_dst_pod = pkt.protocols[5].dst
                    ipv4_src_pod = pkt.protocols[5].src

                    tcp_dstport_pod = pkt.protocols[6].dst_port
                    tcp_srcport_pod = pkt.protocols[6].src_port


                    dict_value = [ipv4_src_pod, ipv4_dst_pod,ipv4_src_host
                            ,ipv4_dst_host,tcp_srcport_pod, tcp_dstport_pod]
                    mydict = {k:v for k,v in zip(dict_key,dict_value)}
                    # self.logger.info("%s", self.kubeSystem)
                    json_merge = json.dumps(mydict)
                    result_post = 'none'
                    ...
                    try:
                        ...
                        self.logger.info("Khong the ket noi toi WebServer")
                        check = False
                    
                    if (str(result_post) == "khongtontai"):
                        check = False
                    if str(result_post) != "khongtontai" and str(result_post) != "none":
                        self.logger.info("Da them o trong Database.Route")
